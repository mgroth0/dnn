...
var BinaryChoice_Base, BinaryChoice_Flash, BinaryChoice_Wait, Instruction, Login, MultiChoice_Base, MultiChoice_Wait, Step, Thank_You, _current_step, _current_step_index, _steps, check_platform, choice_text_center, fsHandler, left_control, push_data, push_data_async, right_control, run_step, run_steps, user,
  indexOf = [].indexOf;
...
MultiChoice_Base = class MultiChoice_Base extends Step {
  constructor(im, t1, choice1, choice2, choice3) {
  ...
  run() {
    ...
    id['text-container'].disappear(); // so the img is centered
    //        id.text.innerHTML = center_str("#{@left_choice} <-?-> #{@right_choice}",'?')
    id.text.innerHTML = `(${control1.str})${this.choice1}, (${control2.str})${this.choice2}, (${control3.str})${this.choice3}`;
    id['im-container'].appear();
    id.cross.appear();
    id.im.src = this.fix_path ? path_join(RESOURCES_ROOT_REL, this.im) : this.im;
    return this.display_stimulus();
  ...
  onkey(e) {
    this.reaction_time_ms = new Date().getTime() - this.presentation_time_unixms;
    return this.choice = ((e.keyCode === control1.keyCode) ? this.choice1 : ((e.keyCode === control2.keyCode) ? this.choice2 : this.choice3));
  }
...
MultiChoice_Wait = class MultiChoice_Wait extends MultiChoice_Base {
  display_stimulus() {
    return setTimeout(() => {
      id.cross.disappear();
      //            setTimeout(=>
